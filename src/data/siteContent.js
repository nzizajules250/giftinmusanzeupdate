export const hotelInfo = {
  name: 'GiftInn',
  city: 'Musanze',
  country: 'Rwanda',
  phone: '+250788608388',
  email: 'martius02@yahoo.fr',
  whatsapp: '250788608388',
  address: 'NM 28 St, Near Musanze City Center, Musanze, Rwanda',
  coordinates: '-1.4992,29.6350',
}

export const navItems = [
  { label: 'Home', path: '/' },
  { label: 'Rooms & Suites', path: '/rooms' },
  { label: 'Booking', path: '/booking' },
  { label: 'About Us', path: '/about' },
  { label: 'Amenities', path: '/amenities' },
  { label: 'Gallery', path: '/gallery' },
  { label: 'Reviews', path: '/reviews' },
  { label: 'Contact', path: '/contact' },
  { label: 'Blog', path: '/blog' },
]

export const uspItems = [
  '5 minutes from Volcanoes National Park routes',
  'Mountain-view rooms with premium comfort',
  'Conference and event spaces for teams',
  'Fast booking with WhatsApp and online payment',
]

export const rooms = [
  {
    id: 1,
    name: 'Deluxe King View',
    price: 15000,
    guests: '2 guests',
    size: '38 m2',
    description:
      'Elegant king room with panoramic mountain light, artisan decor, and a private balcony for sunrise views.',
    image:
      'https://i.ibb.co/qq2KDHF/image-11.jpg',
    images: [
      'https://i.ibb.co/70wmwRh/image-10.jpg',
    ],
    amenities: ['King bed', 'Balcony view', 'High-speed Wi-Fi', 'Breakfast'],
  },
  {
    id: 2,
    name: 'Executive Twin Suite',
    price: 17000,
    guests: '3 guests',
    size: '45 m2',
    description:
      'Business-ready twin suite with premium work desk, lounge area, and comfort-focused night lighting.',
    image:
      'https://i.ibb.co/PGJS4J0v/image-17.jpg',
    images: [
      'https://i.ibb.co/hxV5sGHW/image-16.jpg',
      ],
    amenities: ['Twin premium beds', 'Workspace desk', 'Mini bar', 'Airport pickup'],
  },
  {
    id: 3,
    name: 'Family Panorama Suite',
    price: 22000,
    guests: '4 guests',
    size: '60 m2',
    description:
      'Spacious suite for families with private lounge, connected sleeping spaces, and curated kids amenities.',
    image:
      'https://i.ibb.co/pvGkFQdZ/image-18.jpg',
    images: [
      'https://i.ibb.co/qY5Sjdzx/image-14.jpg',
    ],
    amenities: ['2 bedrooms', 'Private lounge', 'Smart TV', 'Kids package'],
  },
]

export const amenities = [
  { title: 'Spa & Wellness', text: 'Steam, massage, beauty treatments and recovery sessions.' },
  { title: 'Infinity Pool', text: 'Heated pool with sunset mountain skyline.' },
  { title: 'Fitness Studio', text: 'Modern gym with personal coaching options.' },
  { title: 'Fine Dining', text: 'Local and international menu with chef tasting nights.' },
  { title: 'Conference Rooms', text: 'Business-ready halls with hybrid meeting setup.' },
]

export const testimonials = [
  {
    name: 'Aline M.',
    text: 'The rooms were spotless and the staff anticipated everything. Best stay in Musanze.',
    rating: 5,
  },
  {
    name: 'David K.',
    text: 'Our team retreat was smooth from booking to checkout. Conference service was excellent.',
    rating: 5,
  },
  {
    name: 'Noah T.',
    text: 'Great view, peaceful spa, and very fast support on WhatsApp.',
    rating: 4,
  },
]

export const galleryImages = [
  'https://i.ibb.co/qY5Sjdzx/image-1.jpg',
  'https://i.ibb.co/70wmwRh/image-2.jpg',
  'https://i.ibb.co/PGJS4J0v/image-3.jpg',
  'https://i.ibb.co/hxV5sGHW/image-6.jpg',
  'https://i.ibb.co/pvGkFQdZ/image-9.jpg',
  'https://i.ibb.co/qq2KDHF/image-10.jpg',
]

export const blogPosts = [
  {
    title: 'Top 7 Things To Do in Musanze',
    date: '2026-02-01',
    summary: 'Discover gorilla trekking, local markets, and nature trails around GiftInn.',
  },
  {
    title: 'Seasonal Honeymoon Offer',
    date: '2026-01-15',
    summary: 'Romantic package with spa, dining, and late checkout for couples.',
  },
  {
    title: 'Business Retreat Planning Checklist',
    date: '2025-12-20',
    summary: 'Everything your team needs for productive offsite sessions in Musanze.',
  },
]
